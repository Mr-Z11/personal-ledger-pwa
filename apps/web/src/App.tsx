...
async function clearRuntimeCaches() {
  ...
  await Promise.all(keys.filter((key) => key === "ledger-api").map((key) => caches.delete(key)));
}
...
function transactionCategoryName(transaction: Transaction, categories: Category[]) {
  const category = categories.find((entry) => entry.id === transaction.categoryId);
  return category ? categoryPath(category, categories) : "未分类";
}

function transactionTitle(transaction: Transaction, categories: Category[]) {
  return transaction.merchant || transactionCategoryName(transaction, categories) || typeLabels[transaction.type];
}

function isSpecialExpense(transaction: Transaction, categories: Category[]) {
  if (transaction.type !== "expense") return false;
  if (transaction.amountCents >= 100_000) return true;
  const text = [
    transactionCategoryName(transaction, categories),
    transaction.merchant,
    transaction.note,
    transaction.tags.join(" ")
  ].join(" ");
  return /专项|大额|房租|房贷|车贷|保险|医疗|教育|装修|旅行|家电|数码|还款|投资|维修|人情|礼金/.test(text);
}
...
function Overview({ summary, totalAssets, accounts, categories, transactions }: {
  ...
  const currentMonth = monthKey();
  const monthExpenses = transactions.filter((item) => item.type === "expense" && item.occurredAt.startsWith(currentMonth));
  const specialExpenses = monthExpenses.filter((item) => isSpecialExpense(item, categories));
  const specialExpenseCents = specialExpenses.reduce((sum, item) => sum + item.amountCents, 0);
  const dailyExpenseCents = Math.max(0, summary.expenseCents - specialExpenseCents);
  const specialRatio = summary.expenseCents > 0 ? Math.round((specialExpenseCents / summary.expenseCents) * 100) : 0;
  const topSpecialExpenses = [...specialExpenses].sort((left, right) => right.amountCents - left.amountCents).slice(0, 4);
  return (
    <section className="grid overview-grid">
      ...
      <div className="panel overview-expense-panel">
        <div className="section-heading">
          <h2>支出结构</h2>
          <span>专项/大额占 {specialRatio}%</span>
        </div>
        <div className="expense-split">
          <div>
            <span>日常消费</span>
            <strong>¥{centsToYuan(dailyExpenseCents)}</strong>
          </div>
        ...
        <div className="split-bar" aria-label="日常消费与专项大额支出占比">
          <i style={{ width: `${summary.expenseCents > 0 ? Math.max(4, Math.round((dailyExpenseCents / summary.expenseCents) * 100)) : 0}%` }} />
          <b style={{ width: `${summary.expenseCents > 0 ? Math.max(4, specialRatio) : 0}%` }} />
        </div>
        <div className="special-list">
          {topSpecialExpenses.length === 0 && <p className="empty compact">本月暂无专项或大额支出</p>}
          {topSpecialExpenses.map((item) => (
            <div className="special-line" key={item.id}>
              <span>{transactionTitle(item, categories)}</span>
              <small>{new Date(item.occurredAt).toLocaleDateString()}</small>
              <strong>¥{centsToYuan(item.amountCents)}</strong>
            </div>
          ))}
        </div>
      </div>
...
function Reports({ transactions, accounts, categories }: { transactions: Transaction[]; accounts: Account[]; categories: Category[] }) {
  ...
  const selectedCategoryTransactions = selectedCategory
    ? reportTransactions
        .filter((item) => item.type === categoryKind)
        .filter((item) => (item.categoryId ?? "uncategorized") === selectedCategory.id)
    : [];
        ...
        <div className="panel chart-panel category-analysis">
          ...
          {selectedCategory && (
            <div className="category-detail">
              ...
              <TransactionRows transactions={selectedCategoryTransactions.slice(0, 8)} accounts={accounts} categories={categories} compact />
              {selectedCategoryTransactions.length > 8 && <p className="detail-note">仅显示最近 8 笔，可到流水页继续筛选。</p>}
            </div>
          )}
        </div>
